  ...
  return (
    <section className="bg-surface-lowest rounded-xl p-4 md:p-5 border border-border/40 shadow-card">
      ...
      <p className="text-[11.5px] text-muted-foreground mb-4 leading-relaxed">
        ...
        da métrica composta — sem elas o sistema não declara acurácia.
      </p>

      <form
        className="grid grid-cols-1 sm:grid-cols-[110px_150px_1fr_auto] gap-2 items-end"
        onSubmit={(e) => {
          e.preventDefault();
          if (!valido) return;
          add.mutate(
            { alturaCm: valor, measuredAt: data, observacao: obs },
        ...
        <ul className="mt-4 divide-y divide-border/40">
          {measurements.slice(0, 5).map((m) => (
            <li key={m.id} className="flex items-center justify-between py-2 text-[12px]">
              <span className="text-muted-foreground">{formatDateBR(m.measuredAt)} · {m.autor ?? "—"}</span>
